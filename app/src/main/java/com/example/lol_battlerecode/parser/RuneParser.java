package com.example.lol_battlerecode.parser;
            ...
            for (int i=0; i<jsonArray.length(); i++){
                ...
                if (mainObject.get("id").equals(runeId)){
                    ...
                    for (int j=0; j<runesArray.length(); j++){
                        JSONObject runesObject = runesArray.getJSONObject(j);
                        if (runesObject.get("id").equals(runeId)){
                            icon = runesObject.getString("icon");
                            break;
                        }
                    }
                }
